...
COMMON_EXPORT double fraction(const QString &expr, bool *ok = 0);
COMMON_EXPORT QVariant eval(const QString &expr);
COMMON_EXPORT QString samplePrefix(weighbridge::SampleType sampleType);
COMMON_EXPORT double round(double val, int precision = 4, char format = 'g');
COMMON_EXPORT void sleep(int ms);
COMMON_EXPORT int nextSampleNo(const QDate &date);
COMMON_EXPORT void setGeneratedValue(QSqlRecord &rec, const QString &field, const QVariant &value);
COMMON_EXPORT unsigned short crc16(const void *buf, int len);
...
//信息提示函数
COMMON_EXPORT QMessageBox::StandardButton
...
class COMMON_EXPORT BoolLocker
{
public:
...
class COMMON_EXPORT WaitCursor
{
public:
